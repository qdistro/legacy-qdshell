...
Singleton {
  ...
  function load() {
    ...
    Logger.i("ClipboardPolicy",
             "no policy file; default-deny for all cross-silo transfers");
  }
  ...
  function consult(srcSilo, dstSilo, mimeList) {
    if (!root.loaded) {
      return { verdict: "deny", reason: "policy-not-loaded" };
    }
    for (let i = 0; i < root.rules.length; i++) {
      ...
      if (v === "prompt") {
        ...
        return { verdict: "deny", reason: "prompt-collapsed:rule#" + i };
      }
      return { verdict: v, reason: "rule#" + i };
    }
    return { verdict: "deny", reason: "default-deny" };
  }
  ...
  FileView {
      ...
      try {
        ...
        for (let i = 0; i < arr.length; i++) {
          ...
          if (["allow", "deny", "prompt"].indexOf(r.verdict) < 0) {
            Logger.w("ClipboardPolicy", "rule #" + i
                     + " verdict='" + r.verdict + "' invalid; skipping");
            continue;
          }
          cleaned.push(r);
        }
        root.rules = cleaned;
        root.loaded = true;
        Logger.i("ClipboardPolicy", "loaded", cleaned.length, "rules from",
                 root.loadedFrom);
      } catch (e) {
    ...
    onLoadFailed: function (error) {
      Logger.d("ClipboardPolicy", "no JSON policy at", root.loadedFrom,
               "(", error, "); default-deny");
      root.rules = [];
      root.loaded = true;
      root.loadedFrom = "";
  ...
  FileView {
    ...
    printErrors: false
    onLoaded: {
      Logger.w("ClipboardPolicy",
               "YAML policy at",
               _yamlProbe.path,
               "found but Phase-1 ships JSON only; ignoring.",
               "TODO(track-04-phase-2): wire YAML parser.");
    }
    onLoadFailed: function (error) { /* expected absent */ }
  }
  ...
  function _mimeGlobMatch(glob, mime) {
    ...
    return mime.indexOf(before) === 0
        && mime.length >= before.length + after.length
        && mime.substring(mime.length - after.length) === after;
  }
